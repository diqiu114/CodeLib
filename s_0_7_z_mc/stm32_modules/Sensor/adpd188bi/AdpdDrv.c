...
int16_t AdpdDrvRegWrite(uint16_t nAddr, uint16_t nRegValue) {
...
#ifdef ADPD_SPI
  pTxData[0] = (pTxData[0] << 1 ) | ADPD_SPI_WRITE ;             // To set the last bit high for write operation
  if (ADPD_SPI_Transmit(pTxData, 3)!= 1) {
  ...
  */
//  if (ADPD_I2C_Transmit((uint8_t *) pTxData, 3) != 1) {
...
int16_t AdpdDrvRegRead(uint16_t nAddr, uint16_t *pnData) {
...
#ifdef ADPD_SPI
  uint8_t pTxData[1];
  pTxData[0] = ( nRegAddr << 1) | ADPD_SPI_READ;  //   reset the last bit for read operation
  
  if (ADPD_SPI_Receive(pTxData, anRxData, 1, 2)!= 1) {
    return ADPDDrv_ERROR;
  }
#else
  ...
  */
//  if (ADPD_I2C_TxRx(&nRegAddr, (uint8_t *) anRxData, 2) != 1) {
...
int16_t AdpdDrvSetOperationMode(uint8_t nOpMode) {
  ...
  if (nOpMode == ADPDDrv_MODE_IDLE) {
    ...
    nRetCode |= AdpdDrvRegRead(REG_DATA_BUFFER, &nTemp); // Read 2 bytes dummy FIFO data
...
int16_t AdpdDrvGetParameter(AdpdCommandStruct eCommand, uint16_t *pnValue) {
  ...
  if (eCommand == ADPD_WATERMARKING) {
    ...
    nRegValue1 = gaGetSlotSize[nRegValue1];   /* Get SlotA size */
    nRegValue2 = gaGetSlotSize[nRegValue2];   /* Get SlotB size */
    nRegValue1 = nRegValue1 + nRegValue2;   /*Total slot size */
...
int16_t AdpdDrvReadFifoData(uint8_t *pnData, uint16_t nDataSetSize) {
  ...
  if (gnDeviceID < DEVICE_ID_516) {
    if (gnFifoLevel >= nDataSetSize) {
      nGetFifoData = 1;
      // enable FIFO clock
      /*if (AdpdDrvRegWrite(REG_FIFO_CLK, FIFO_CLK_EN) == ADPDDrv_ERROR) {
      return ADPDDrv_ERROR;
    }
      // enable FIFO clock twice
      if (AdpdDrvRegWrite(REG_FIFO_CLK, FIFO_CLK_EN) == ADPDDrv_ERROR) {
      return ADPDDrv_ERROR;
    }*/
      // kfkf remove fifo access noise
  ...
  if (gnFifoLevel >= nDataSetSize) {
    ...
    */
    if (ADPD_I2C_TxRx((uint8_t*) &nAddr, pnData, nDataSetSize) != ADI_HAL_OK) {
                        return ADPDDrv_ERROR;
                      }
#endif // ADPD_SPI
  }
...
//  switch (moduletype) {
//  case 1:
//    return (uint8_t*)("153GGRI");
//  case 2:
//    return (uint8_t*)("163URI");
//  case 3:
//    return (uint8_t*)("163URIR2");
//  case 4:
//    return (uint8_t*)("163URIR3");
//  case 5:
//    return (uint8_t*)("174GGI");
//  default:
//    return (uint8_t*)("153GGRI");  // unknown device is detected
...
//#ifdef FOUR_HZ_FOUR_MEAN_FOR_PULESE 
 // nAdpdTxData=0x0220;  //算术平均数4
//#endif
//#ifdef EIGHT_HZ_EIGHT_MEAN_EIGHT_PULESE
//  nAdpdTxData=0x0330;  //算术平均数8
//#endif 
//#ifdef TWO_HZ_TWO_MEAN_TWO_PULESE
//  nAdpdTxData=0x0110;  //算术平均数2
//#endif
...
int16_t AdpdDrvSoftReset(void) {
  if(AdpdDrvRegWrite(REG_SW_RESET, 0x1) != ADPDDrv_SUCCESS)
  {
    //printf("SoftReset ERROR");
    return ADPDDrv_ERROR;
  }
  else
  {
    //printf("SoftReset SUCCESS");
    return ADPDDrv_SUCCESS;
  }
}
...
/** @brief Get the device Slot Mode in the REG_OP_MODE_CFG Reg
*
* @param        nSlotId:   1 --> SLOT_A
*                          2 --> SLOT_B
* @param        pRegValue pointer to which the Slot Mode value will be loaded
* @return int16_t A 16-bit integer: 0 - success; < 0 - failure
*/
static int16_t GetSlotMode(uint8_t nSlot, uint16_t *pRegValue)
{
