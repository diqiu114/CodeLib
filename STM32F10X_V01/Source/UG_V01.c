...
{
  UG_Key_DoEvent_IMM();
  if(!IMM_Event_On)UG_DT_RefreshMiniSecond();
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_APP_INT_Event(void)
{
  #if SDK_COM_COUNT
  if(Read_DPStr_On)UG_Read_DPStr();
	if(SendState_On)UG_Send_State();
	if(Device_PWSet_On)UG_SetPWState();
  #endif
...
{
  UG_APP_TMR(1);TMRCount[1]++;if(TMRCount[1]<2) return;TMRCount[1]=0x00;
  UG_APP_TMR(2);TMRCount[2]++;if(TMRCount[2]<10)return;TMRCount[2]=0x00;
  UG_APP_TMR(3);TMRCount[3]++;if(TMRCount[3]<10)return;TMRCount[3]=0x00;
  UG_APP_TMR(4);
}
//------------------------------------------------------------------------------------------------------------------------------
...
{
  if(Offset+sLen>UG_AppPara_Len)return;
  UG_Flash_WriteItem((UINT32)&UG_AppPara+Offset,s,sLen); 
}  
//------------------------------------------------------------------------------------------------------------------------------
void UG_Para_ReadItem(UINT16 Offset,UINT8 *s,UINT16 sLen)
{
  if(Offset+sLen>UG_AppPara_Len)return;
  if(!DC_Inited && UG_Flash_IsNew())UG_Para_Write();UG_Flash_ReadString((UINT32)&UG_AppPara+Offset,s,sLen);  
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_Para_Default(void)
{
  DAPP.INTF.IMM_Event=UG_APP_IMM_Event;
  DAPP.INTF.INT_Event=UG_APP_INT_Event;
  DAPP.INTF.TMR_Event=UG_APP_TMR_Event;
  DAPP.INTF.TMR=UG_APP_TMR;
  DAPP.INTF.Init=UG_APP_Init;
  DAPP.INTF.IRQ_TIM=UG_APP_IRQ_TIM;
  #if SDK_COM_COUNT
  DAPP.INTF.COM_ReadPacket=UG_COM_ReadPacket;
  DAPP.INTF.COM_ReadParameter=UG_COM_ReadParameter;
  #endif
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_Para_Write(void)
{
  ...
  UG_Para_WriteItem(0,DAPP.TBUF,AppPara_Len);
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_Para_Read(void)
{
  UG_Para_ReadItem(0,DAPP.TBUF,AppPara_Len);
}
//------------------------------------------------------------------------------------------------------------------------------
...
{
  DDEV.Item[0].RunCount++;UG_ValueToStr(DAPP.TBUF,DDEV.Item[0].RunCount,4); UG_Para_WriteItem(AppPara_RunCount,DAPP.TBUF,4);  
}
//==============================================================================================================================
...
void UG_Para_Save_Timer(void)//every 100ms
{
  ...
  Para_Save_TimeCount++;if(Para_Save_TimeCount>=UG_AppPara_Area_DelayTime[AppPara_Area]){Para_Save_On=0;UG_Para_Write();}//10s
}
//==============================================================================================================================
...
{
  if(Sleep_On)return;  SleepTimeCount++; if(SleepTimeCount>=100)UG_SetSleepOn(1);
}
//==============================================================================================================================
...
//-----------------------------------------------------------------------------------------------------------------------------
UINT8 UG_Key_KB_GetCode(unsigned char ScanState)
{
  #if SDK_KB_On
...
void UG_Key_RX_Timer(void)//every 5ms
{
  ...
  INT[1]=DKEY.TempState^DKEY.ScanState;INT[0]=1;
	for(Var[0]=0;Var[0]<SDK_Key_Count;Var[0]++)
	{
        ...
        else
        {
          ...
          {
					  DKEY.DownState^=INT[0];
						if(!((Key_LongDown_EN&INT[0]) && (DKEY.LongDownState&INT[0])==0) && !((Key_Repeat_EN&INT[0]) &&  (DKEY.RepeatState&INT[0])==0))	UG_Key_Save(Var[0],Key_EV_Press);
					}
					UG_Key_Save(Var[0],(DKEY.LongDowned & INT[0] ? Key_EV_LongUp:Key_EV_Up));
        }
        ...
        {
          DKEY.LongDowned|=INT[0];UG_Key_Save(Var[0],Key_EV_LongDown+DKEY.LongTimeIndex[Var[0]]);DKEY.LongTimeIndex[Var[0]]++;
          if(DKEY.LongTimeIndex[Var[0]]>=Key_LongTime_Count)DKEY.LongDownState^=INT[0];
			...
			if(Key_Repeat_EN&INT[0])
			{
				...
				if(DKEY.RepeatTimeCount[Var[0]]>=(DKEY.RepeatState&INT[0] ? Key_Repeat_FirstTime:Key_Repeat_Time)){DKEY.RepeatTimeCount[Var[0]]=0x0000;UG_Key_Save(Var[0],Key_EV_Press);}		
			}
		}
		INT[0]<<=1;
	}
  ...
  for(Var[0]=0;Var[0]<Key_CC_Count;Var[0]++)
  {
    ...
    if(INT[0]==Key_CC[Var[0]])
    {
      ...
      {UG_Key_Save(Var[0]+SDK_Key_Count,Key_EV_CDown);DKEY.CEN[Var[0]]=(DKEY.CEN[Var[0]]&0xFE)|0x04;}
      else if(Key_LongTime_Count && (DKEY.CEN[Var[0]]&2) )//LongPress
      {
        if(DKEY.CTimeCount[Var[0]]>=Key_LongTime[DKEY.CLongTimeIndex[Var[0]]])
        {
          UG_Key_Save(Var[0]+SDK_Key_Count,Key_EV_LongDown+DKEY.CLongTimeIndex[Var[0]]);DKEY.CLongTimeIndex[Var[0]]++;
    ...
    else
    {
      ...
      if(DKEY.CEN[Var[0]]&4){UG_Key_Save(Var[0]+SDK_Key_Count,Key_EV_CUp);DKEY.CEN[Var[0]]=0x00;}
  ...
  if(INT[1]&(0xFFFF>>(16-SDK_KB_RowCount)) && INT[1]&(((1<<SDK_KB_ColCount)-1)<<SDK_KB_RowCount))
  {
    ...
    if(DKEY.KB_TimeCount>=Key_DebounceTime)
    {
      ...
      if(UG_Key_KB_GetCode(INT[1]))
      {
        ...
        UG_Key_Save(SDK_Key_Count+Key_CC_Count+DKEY.KB_Code,(TempFlag0 ? Key_EV_Down:(Key_KB_LongDowned ? Key_EV_LongUp:Key_EV_Up)));
      }
      else
      {Key_KB_Down_On=0;}
  ...
  else
  {
    ...
    if(Key_KB_Down_On)
    {
        ...
        {
          Key_KB_LongDowned=1;UG_Key_Save(SDK_Key_Count+Key_CC_Count+DKEY.KB_Code,Key_EV_LongDown+DKEY.KB_LongTimeIndex);DKEY.KB_LongTimeIndex++;
          if(DKEY.KB_LongTimeIndex>=Key_LongTime_Count)Key_KB_LongDown_On=0;
			...
			if(SDK_KB_Repeat_EN)
			{
				...
				if(DKEY.KB_RepeatTimeCount>=(Key_KB_Repeat_State ? Key_Repeat_FirstTime:Key_Repeat_Time)){DKEY.KB_RepeatTimeCount=0x00;UG_Key_Save(SDK_Key_Count+Key_CC_Count+DKEY.KB_Code,Key_EV_Press);}		
			}      
    }
  }   
  #endif
...
//------------------------------------------------------------------------------------------------------------------------------
void UG_PVR_RX_Timer(void)//every 1ms
{
  #if SDK_PVR_Count
  ...
  for(IRQVar[0]=0;IRQVar[0]<SDK_PVR_Count;IRQVar[0]++)
  {
    if(DKEY.PVR_TempState&IRQINT[0])
    {
      ...
      for(IRQVar[1]=0;IRQVar[1]<SDK_PVR_UDCount;IRQVar[1]++){if((UG_PVR_UD[IRQVar[0]][IRQVar[1]]&0xFF)==DKEY.PVR_State[IRQVar[0]])break;}
      if(IRQVar[1]<SDK_PVR_UDCount) UG_Key_Save(IRQVar[0]+SDK_Key_Count-SDK_PVR_Count,(PVR_UD[IRQVar[0]][IRQVar[1]]&0x100 ? Key_EV_ScrollDown:Key_EV_ScrollUp));
    }
    IRQINT[0]<<=2;
  }
  #endif  
...
//------------------------------------------------------------------------------------------------------------------------------
void UG_Key_Save(UINT8 Code,UINT8 EV)
{
...
{
  UG_SetDeviceOn(!Device_On);
}
//==============================================================================================================================
#if SDK_COM_COUNT
void UG_COM_Connect_TurnOn(UINT16 Index)
{
	if(!COM_Connect_On(Index)){COM_Connect_On(Index)=1;UG_Event_Clear();} 
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_COM_Connect_TurnOff(UINT16 Index)
{
	if(COM_Connect_On(Index)){COM_Connect_On(Index)=0;UG_Event_Clear();}
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_COM_Connect_Timer(void)//every 10ms
{
  for(Var[0]=0;Var[0]<DCOM.Count;Var[0]++)
  {if(COM_Connect_On(Var[0]) && SysTimeCount-DCOM.Item[Var[0]].RXTime>=1000){COM_Connect_On(Var[0])=0;UG_Event_Clear();}}
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_COM_VerifySend(void)
{
	DAPP.TF.Verify=0x00;for(Var[0]=6;Var[0]<DAPP.TF.Count;Var[0]++)DAPP.TF.Verify+=DAPP.TF.Item[Var[0]];
	DAPP.TF.Item[DAPP.TF.Count++]=DAPP.TF.Verify;UG_COM_SendString(DAPP.TF.PortIndex,DAPP.TF.Item,DAPP.TF.Count,0);
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_COM_Save_BUS(UINT16 Index)//保存绑定
{
}
//==============================================================================================================================
void UG_COM_Packet_Timer(void)//every 1ms
{
  ...
  for(i=0;i<SDK_COM_COUNT;i++)
  {
    ...
    {if(DCOM.Item[i].PRTC.Count==0){UG_COM_ReadParameter_EMPTY(i,1);} UG_COM_ClearRX(i,true);}
    else
    {IRQTempFlag0=1;}
  }
...
void UG_COM_ReadPacket(UINT16 Index,UINT8* s,UINT16 Count)
{
  ...
  {
    UG_COM_ReadPacket_EMPTY(Index,s,Count);
  }
  else
  {
    switch(DCOM.Item[Index].PRTC.Item[DCOM.Item[Index].PRTC.Index])
    {
      case COM_Protocol_LDY_V2: UG_COM_ReadPacket_LDY_V2(Index,s,Count);break;
      case COM_Protocol_DMX512: UG_COM_ReadPacket_DMX512(Index,s,Count);break;
      default:UG_COM_ReadPacket_EMPTY(Index,s,Count);break;
    }
  }
}
//------------------------------------------------------------------------------------------------------------------------------
...
void UG_COM_ReadPacket_DMX512(UINT16 Index,UINT8* s,UINT16 Count)
{
  ...
  for(i=0;i<Count;i++)
    ...
    if(COM_Bit9(Index)==1)
    {
      ...
      if((*RF).Count>=512){UG_COM_ReadParameter_DMX512(Index,(*RF).Item,(*RF).Count);UG_COM_ClearRX(Index,true);}
    }
    else if((*RF).Count)
    {UG_COM_ReadParameter_DMX512(Index,(*RF).Item,(*RF).Count);UG_COM_ClearRX(Index,true);}
  }
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_COM_ReadPacket_LDY_V2(UINT16 Index,UINT8* s,UINT16 Count)
{
  ...
  RF=&DCOM.Item[Index].RF;
	...
	{
		if(!(s[0]==COM_Head_LDY[0] && s[1]==COM_Head_LDY[1])){UG_COM_RXNoise(Index,0,Count);return;}
		(*RF).Control=s[2];(*RF).Command =s[3];(*RF).ParaCount=(Count==4 ? 0:(s[4]|(UINT16)s[5]<<8));
    if((s[2]&Control_SF) && Count==4){UG_COM_ReadParameter_LDY_V2(Index,s+6,(*RF).ParaCount);return;}
    if(Count>=(*RF).ParaCount+7)	(*RF).Verify=0x00; else return;
		for(i=0;i<(*RF).ParaCount;i++){(*RF).Verify+=s[6+i];}
		if((*RF).Verify==s[6+i])UG_COM_ReadParameter_LDY_V2(Index,s+6,(*RF).ParaCount); else UG_COM_RXNoise(Index,0,Count);
	}
	else//无硬件打包
	{
		for(i=0;i<Count;i++)
		{
			...
			{
				if(UG_COM_IsHead(Index,DCOM.Item[Index].RF.Index,(UINT8 *)COM_Head_LDY,SIZE_COM_HeadF_LDY))
				{
					UG_COM_RXNoise(Index,((*RF).Index +(*RF).Size - SIZE_COM_HeadF_LDY ) % (*RF).Size, (*RF).Count - SIZE_COM_HeadF_LDY );
					COM_RBUF_HeadOn(Index)=COM_Head_On=1;
					COM_RBUF_ParaCountOn(Index) =COM_RBUF_LFOn(Index)=0;
					UG_COM_ClearRX(Index,false);
				}
			}
			else if(!COM_RBUF_ParaCountOn(Index))
			{
				if((*RF).Count==2)//Control+Command(2 bytes)
				{
					...
					{(*RF).ParaCount =0;UG_COM_ReadParameter_LDY_V2(Index,(*RF).Item,(*RF).ParaCount);UG_COM_ClearRX(Index,true);}
				}
			...
			else if(COM_RBUF_LocalOn(Index)) //is myself packet(自己的包，包含中继包与非中继包)
			{
				if( !((*RF).Control&Control_AD) || (*RF).ADR.Item[3]==((*RF).Control&Control_BG ? 0:(*RF).ADR.Item[1]+1) )//非中继
				{
					...
					if(COM_RBUF_LFOn(Index))
					{
						...
						{
							if((*RF).Item[(*RF).Index -1]==(*RF).Verify) {(*RF).LFSubParaCount -= 1;UG_COM_RXSubFrame(Index,true);}//长帧校验 OK
							UG_COM_ClearRX(Index,true);
						}
						else if ((*RF).LFSubParaCount >= (*RF).Size)
						{
							UG_COM_RXSubFrame(Index,false);(*RF).LFParaCount += (*RF).LFSubParaCount;(*RF).LFSubParaCount = 0;UG_COM_ClearRX(Index,false);
						}
					}			
					else if((*RF).Count>(*RF).ParaCount)
					{
						if((*RF).Item[(*RF).Index -1]==(*RF).Verify)UG_COM_ReadParameter_LDY_V2(Index,(*RF).Item,(*RF).ParaCount);//普通校验 OK
						UG_COM_ClearRX(Index,true);
					}
				...
				{		
					if(COM_RBUF_LFOn(Index))//中继长包
					{
						...
						if((*RF).LFParaCount+(*RF).LFSubParaCount>(*RF).ParaCount)
						{UG_COM_TXRelayFrame(Index,true);UG_COM_ClearRX(Index,true);}
						else if((*RF).Count >=(*RF).Size)
						{UG_COM_TXRelayFrame(Index,false);UG_COM_ClearRX(Index,false);(*RF).LFParaCount+=(*RF).LFSubParaCount;(*RF).LFSubParaCount=0x00;}
					}
					else if(DCOM.Item[Index].RF.Count>DCOM.Item[Index].RF.ParaCount)//中继普通包
					{(*RF).LFSubParaCount=(*RF).Count;UG_COM_TXRelayFrame(Index,true);UG_COM_ClearRX(Index,true);}
				}//中继与非中继结束	
			}			
			else //other frame （别人的包，旁路)
			{(*RF).LFParaCount +=0x01;	if((*RF).LFParaCount > (*RF).ParaCount ) UG_COM_ClearRX(Index,true);}
		}
	}
}
//------------------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------------------
UINT16 UG_COM_AddrToPort(UINT16 Index)//根据目标地址求端口（必须针对产品修改此函数),一般可以用高位表示端口编码地址，针对这种情况，可以处理此函数；当然也可以把端口号当成单独的地址，路径就会多一级。
{
  return(0);
}
//------------------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------------------
void UG_COM_TXRelayFrame(UINT16 Index,UINT8 IsEnd)//中继发送(中继不存在校验和解密,数据直接转发)
{
  ...
  if(DCOM.Item[Index].RF.LFParaCount==0x00)
    ...
    UG_COM_SendString(DCOM.Item[Index].PORT.O,DAPP.IRQTF.Item, DAPP.IRQTF.Count,DAPP.IRQTF.Time);//Head
    UG_COM_SendString(DCOM.Item[Index].PORT.O,DCOM.Item[Index].RF.ADR.Item,DCOM.Item[Index].RF.ADR.Len,DAPP.IRQTF.Time);//Address List
	}
  else if(!COM_RelayPortOn(Index))
  {return;}
  UG_COM_SendString(DCOM.Item[Index].PORT.O,DCOM.Item[Index].RF.Item, DCOM.Item[Index].RF.LFSubParaCount,DAPP.IRQTF.Time);
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_COM_RXNoise(UINT16 Index,UINT8 Pos,UINT16 Count)
{
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_COM_RXSubFrame(UINT16 Index,UINT8 IsEnd)//长帧子包，Payload subframe,(SP.RXBA.Item, SP.RXFM.SubParaCount)
{
...
void UG_COM_ReadParameter(UINT16 Index,UINT8 *s,UINT16 Count)
{
  if(DCOM.Item[Index].PRTC.Count==0)//无协议通讯
  {
    UG_COM_ReadParameter_EMPTY(Index,1);
  }
  else
  {
    switch(DCOM.Item[Index].PRTC.Item[DCOM.Item[Index].PRTC.Index])
    {
      case COM_Protocol_LDY_V2: UG_COM_ReadParameter_LDY_V2(Index,s,Count);break;
      case COM_Protocol_DMX512: UG_COM_ReadParameter_DMX512(Index,s,Count);break;
      default:UG_COM_ReadParameter_EMPTY(Index,1);break;
    }
  }
}
//==============================================================================================================================
void UG_COM_ReadParameter_EMPTY(UINT16 Index,UINT8 IsEnd)//中断内调用
{
...
{
  if(!Device_On||Count<10)return;UG_SetSleepOn(0);
}
//==============================================================================================================================
void UG_COM_ReadParameter_LDY_V2(UINT16 Index,UINT8 *s,UINT16 Count)//普通包，payload=(s, Count), 
{
  ...
  {DCOM.Item[Index].PORT.App=UG_ByteToValue(s,2,0);DAPP.IRQTF.Item[ParaPos++]=s[0];DAPP.IRQTF.Item[ParaPos++]=s[1];s+=2;Count-=2;}//Control.bit5=1, 实参前两个字节为App端口号
          ...
          case Command_DP_PW:
            break;
					...
					case Command_DP_FrameSize:
						if(RW!=Control_R)break;
            IRQINT[0]=UG_COM_BUFSize(Index,0);IRQINT[1]=UG_COM_BUFSize(Index,1);
          ...
          case Command_DP_DT:
				    ...
				    switch(s[1])
            {
              case Command_DP_DT_BDT:
								if(RW==Control_R)
								{UG_DTToByte8(DAPP.BaseDT,DAPP.IRQTF.Item+ParaPos,DateTime_Mode_Struct);DAPP.IRQTF.ParaCount=8;IRQTempFlag1=1;}								
								else if(Count==10)
								{UG_Byte8ToDT(s+2,&DAPP.BaseDT,DateTime_Mode_Struct);IRQTempFlag1=1;Write_BaseDT_On=INT_Event_On=1;}
								break;                
   					  case Command_DP_DT_MDT:
								if(RW==Control_R)
								{UG_DTToByte8(DAPP.MadeDT,DAPP.IRQTF.Item+ParaPos,DateTime_Mode_Struct);DAPP.IRQTF.ParaCount =8;IRQTempFlag1=1;}
								else if(Count==10)
								{UG_Byte8ToDT(s+2,&DAPP.MadeDT,DateTime_Mode_Struct);IRQTempFlag1=1;Write_MadeDT_On=INT_Event_On=1;}
								break;							
					    case Command_DP_DT_CDT:
						    if(RW==Control_R)
						    {UG_DTToByte8(DAPP.DT,DAPP.IRQTF.Item+ParaPos,DateTime_Mode_Struct); DAPP.IRQTF.ParaCount=8;IRQTempFlag1=1;}
						    else if(Count==10)
					      {UG_Byte8ToDT(s+2,&DAPP.DT,DateTime_Mode_Struct);DT_RefreshMiniSecond(); IRQTempFlag1=1;DTSync_On=1;}
                break;
              default:
                break;
            }
          ...
          case Command_DP_BUS://总线，包括联机前后
            ...
            switch(s[1])
            {
              ...
              case Command_DP_BUS_SetJoin://设置Join
                ...
                {if(UG_StrCompare(DDEV.Item[0].NSN.Item,s+2+IRQVar[0]*DDEV.Item[0].NSN.Count,DDEV.Item[0].NSN.Count,1)==1){COM_BUS_Joined(Index)=1;break;}}
                break;
              case Command_DP_BUS_ClrRel://BUS可以存盘或不存盘[COM_BUS_Changed(Index)]
                if(!COM_BUSL_On(Index)||DDEV.Item[0].NSN.Count==0x00)break;
                if(!(DCOM.Item[Index].RF.ADR.Item[4]==DCOM.Item[Index].BUS.NSNLen && UG_StrCompare(DCOM.Item[Index].BUS.NSN,DCOM.Item[Index].RF.ADR.Item+5,DCOM.Item[Index].BUS.NSNLen,1)==1))  break;//只有绑定该节点的主节点才能对它解绑
                for(IRQVar[0]=0;IRQVar[0]<(Count-2)/DDEV.Item[0].NSN.Count;IRQVar[0]++)
                {if(UG_StrCompare(DDEV.Item[0].NSN.Item,s+2+IRQVar[0]*DDEV.Item[0].NSN.Count,DDEV.Item[0].NSN.Count,1)==1){COM_BUSL_On(Index)=0;COM_BUS_Changed(Index)=1;INT_Event_On=1;break;}}
                break;
              case Command_DP_BUS_SetRel://绑定总线下自由节点，子节点获得平行节点个数及本节点在子节点列表中的索引,子节点可以不存储平行子节点，总节点必须存储子节点。
                ...
                { 
                  ...
                  if((IRQVar[0]%DDEV.Item[0].NSN.Count)==0x00 && UG_StrCompare(DDEV.Item[0].NSN.Item,s+2+IRQVar[0],DDEV.Item[0].NSN.Count,1)==1)
                  {UG_StrCopy(DCOM.Item[Index].BUS.NSN,DCOM.Item[Index].RF.ADR.Item+5,DCOM.Item[Index].RF.ADR.Item[4]);DCOM.Item[Index].BUS.Index=IRQVar[0]/DDEV.Item[0].NSN.Count;COM_BUSL_On(Index)=1;}//获取本节点绑定索引
                }
              ...
              case Command_DP_BUS_GetFree://查询总线下自由节点，准备重新绑定子集
                ...
                DAPP.IRQTF.Time=UG_LdyRand(IRQLNG[0])*3;IRQTempFlag1=1;
                break;
              default:
                break;
            }
...
void UG_COM_SetBack(UINT16 Index)
{
  ...
  if(DAPP.IRQTF.Control&Control_SF){UG_COM_SendString(DAPP.IRQTF.PortIndex,DAPP.IRQTF.Item,4,DAPP.IRQTF.Time); COM_TX_InIRQ(Index)=0;return;}
  ...
  if(DAPP.IRQTF.Control & Control_AD && DCOM.Item[Index].RF.ADR.Item[2]!=1)
  {
	...
	{
	  UG_COM_SendString(DAPP.IRQTF.PortIndex,DAPP.IRQTF.Item,6,DAPP.IRQTF.Time);
    COM_TXContinue_On(DAPP.IRQTF.PortIndex)=1; 
    UG_COM_SendString(DAPP.IRQTF.PortIndex,DCOM.Item[Index].RF.ADR.Item,DCOM.Item[Index].RF.ADR.Len,DAPP.IRQTF.Time);//Address
	  UG_COM_SendString(DAPP.IRQTF.PortIndex,DAPP.IRQTF.Item+6,DAPP.IRQTF.ParaCount+1,DAPP.IRQTF.Time);
    COM_TXContinue_On(DAPP.IRQTF.PortIndex)=0;
	}
	else
	{UG_COM_SendString(DAPP.IRQTF.PortIndex,DAPP.IRQTF.Item,DAPP.IRQTF.ParaCount+7,DAPP.IRQTF.Time);}
...
{
  Device_PWSet_On=0;UG_SetDeviceOn(DAPP.PF.Item[0]);
}
//------------------------------------------------------------------------------------------------------------------------------
...
void UG_Send_State(void)
{
  ...
  for(Var[0]=0;Var[0]<4;Var[0]++)	DAPP.TF.Item[Var[0]+7]=DDEV.Item[0].RunST.Byte[Var[0]];
	DAPP.TF.Count=11;	UG_COM_VerifySend();
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_Write_BaseDT(void)
{
  Write_BaseDT_On=0; UG_DTToByte8(DAPP.BaseDT,DAPP.TF.Item,DateTime_Mode_Simple);
  UG_Flash_WriteItem((UINT32)(Device_BaseDT),DAPP.TF.Item,8);
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_Write_MadeDT(void)
{
  Write_MadeDT_On=0; UG_DTToByte8(DAPP.MadeDT,DAPP.TF.Item,DateTime_Mode_Simple);
  UG_Flash_WriteItem((UINT32)(Device_MadeDT),DAPP.TF.Item,8);
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_Write_HS(void)
{
  Write_HS_On=0;
  UG_Flash_WriteItem((UINT32)(Device_HS),DDEV.Item[0].HS.ST.Byte,4);
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_Write_NSN(void)
{
  Write_NSN_On=0;
  UG_Flash_WriteItem((UINT32)(Device_NSN),DDEV.Item[0].NSN.Item,DDEV.Item[0].NSN.Count);
}
#endif
//==============================================================================================================================
