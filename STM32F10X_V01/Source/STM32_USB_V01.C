...
USB_RESULT Standard_ClearFeature(void)//Clear or disable a specific feature.
{
  ...
  else if (USB_Type_Recipient == (USB_STANDARD_REQUEST | USB_ENDPOINT_RECIPIENT))
  {
    ...
    if ( DUSB.Device_Info.USBwIndexs.LH.L & 0x80)
    {
      ...
      {
				if(USB.EPR_BIT[DUSB.REP].TXDTOG)USB_ToggleDTOG(DUSB.REP,USB_RXTXFlag_TX);
    ...
    else
    {
			if(USB.EPR_BIT[DUSB.REP].RXSTAT==USB_EPR_STAT_STALL) 
      {
        ...
        {if(USB.EPR_BIT[DUSB.REP].RXDTOG)USB_ToggleDTOG(DUSB.REP,USB_RXTXFlag_RX);}
...
void USB_CTR_LP(void)//端点低优先级中断处理
{
	while(USB.ISTR_BIT.CTR)	
  {
    ...
    else//EPindex ！= 0
    {
      ...
      {USB.EPR[DUSB.EPIndex]&=0x8F0F; if(DUSB.EPIndex==USB_TXEPIndex){COM_TXItem_On(SDK_USB_Index)=0;DCOM[SDK_USB_Index].TF.TXI.TimeOut_Count=0x00;if(DCOM[SDK_USB_Index].TF.Count==0x0000)COM_TX_On(SDK_USB_Index)=0;}}
    }
  }
}
//------------------------------------------------------------------------------------------------------------------------------
...
void USB_ReadBUF(UINT8 EPIndex)
{
  ...
  DAPP.IRQ_ReadParameter(SDK_UART_Index,DCOM[SDK_USB_Index].RF.Item,DCOM[SDK_USB_Index].RF.ParaCount);
}	
//------------------------------------------------------------------------------------------------------------------------------
void USB_SendBUF(UINT8 EPIndex)//发送BUF（中断内外调用）
{
  ...
  i=0x0000;while(USB.EPR_BIT[USB_TXEPIndex].TXSTAT!=USB_EPR_STAT_NAK && i<0x200)i++;
	P=(UINT16 *)DUSB.TXADDR[EPIndex];	j=(DCOM[SDK_USB_Index].TF.Index+SIZE_USB_TBUF-DCOM[SDK_USB_Index].TF.Count)%SIZE_USB_TBUF;
  ...
  {Temp=DCOM[SDK_USB_Index].TF.Item[j];j=(j+1)%SIZE_USB_TBUF;	Temp+=((UINT16)DCOM[SDK_USB_Index].TF.Item[j]<<8);j=(j+1)%SIZE_USB_TBUF;*P=Temp;P+=2;}
	DCOM[SDK_USB_Index].TF.Count-=(Len+2);
    
  if(DCOM[SDK_USB_Index].TF.Count==0)
  {DCOM[SDK_USB_Index].TF.MMT.Count=0;}
  else if(DCOM[SDK_USB_Index].TF.MMT.Count>1 && j==DCOM[SDK_USB_Index].TF.MMT.Item[1].Pos)
  {for(i=0;i<DCOM[SDK_USB_Index].TF.MMT.Count-2;i++)DCOM[SDK_USB_Index].TF.MMT.Item[i]=DCOM[SDK_USB_Index].TF.MMT.Item[i+1];DCOM[SDK_USB_Index].TF.MMT.Count--;}
    
	DCOM[SDK_USB_Index].TF.TXI.TimeOut_TimeCount=0x00;DCOM[SDK_USB_Index].TF.TXI.TimeOut_Time=30; COM_TXItem_On(SDK_USB_Index)=1;//Tx Timeout minitor=30*100us=3ms
...
{
	UINT32 T;
	T=DDEV.TimeCount;	while((COM_TX_On(SDK_USB_Index)||COM_TXItem_On(SDK_USB_Index)) && DDEV.TimeCount-T<1000);
}
//------------------------------------------------------------------------------------------------------------------------------
void USB_HW_SendString(UINT8 *s,UINT16 Count)//发送到硬件（中断内外调用）
{
  ...
  T=DDEV.TimeCount;while(USB.EPR_BIT[USB_TXEPIndex].TXSTAT!=USB_EPR_STAT_NAK && DDEV.TimeCount-T<2); 
	...
	DCOM[SDK_USB_Index].TF.TXI.TimeOut_TimeCount=0x00;DCOM[SDK_USB_Index].TF.TXI.TimeOut_Time=30;COM_TX_On(SDK_USB_Index)=1;COM_TXItem_On(SDK_USB_Index)=1;//Tx Timeout minitor=30*100us=3ms
}
//------------------------------------------------------------------------------------------------------------------------------
void USB_TXTimeOut_Timer(void)//every 100us,send timeout control
{
	if(!COM_TXItem_On(SDK_USB_Index))return;
	DCOM[SDK_USB_Index].TF.TXI.TimeOut_TimeCount++; 
  if(DCOM[SDK_USB_Index].TF.TXI.TimeOut_TimeCount>DCOM[SDK_USB_Index].TF.TXI.TimeOut_Time)
  {
		COM_TXItem_On(SDK_USB_Index)=0;if(DCOM[SDK_USB_Index].TF.Count==0x0000)COM_TX_On(SDK_USB_Index)=0;
	  DCOM[SDK_USB_Index].TF.TXI.TimeOut_Count++;if(DCOM[SDK_USB_Index].TF.TXI.TimeOut_Count>=5){USB_Reset_On=1;INT_Event_On=1;}
	}
}
//------------------------------------------------------------------------------------------------------------------------------
void USB_Send_Timer(void)//发送定时事件,every 1ms(中断内外调用）
{
  ...
  if(DCOM[SDK_USB_Index].TF.Count==0x00||DCOM[SDK_USB_Index].TF.MMT.Item[0].Time)COM_EndSend(SDK_USB_Index); else USB_SendBUF(USB_TXEPIndex);
}
//------------------------------------------------------------------------------------------------------------------------------
#endif
//==============================================================================================================================
