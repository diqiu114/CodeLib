...
#include "Global.h"
//==============================================================================================================================
const UINT8             COM_Head_LDY[4]={0x55,0xAA,0x00,0x00},COM_Head_LS[4]={0x5A,0xA5,0x0D,0x0A},COM_Head_YY[2]={0x7F,0x0A};
const UINT8             CRLF[2]={13,10};
#if LdyEXPLOG_On||String_On
const double Double_EXPT[]={//10^(1,2,...9),10^(10,20,...90),10^(100,200,...900),10^(1000,2000,...9000),10^(10000,20000,...90000)
                        1e1,1e2,1e3,1e4,1e5,1e6,1e7,1e8,1e9,
                        1e10,1e20,1e30,1e40,1e50,1e60,1e70,1e80,1e90,
                        1e100,1e200,1e300
                        };
const double Double_EXPE[]={//e^(1,2,...9),e^(10,20,...90),e^(100,200,...900),e^(1000,2000,...9000),e^(10000,20000,...90000)
...
#endif
#if DateTime_On
const UINT8 DateTime_MD[12]={31,28,31,30,31,30,31,31,30,31,30,31};
#endif
//==============================================================================================================================
#if Delay_On
...
{
  if(IsStruction) Delay_MS(Time); else Delay_TimerTime(Time);
}
#endif
//==============================================================================================================================
#if String_On
...
//-----------------------------------------------------------------------------------------------------------------------------
UINT8 StrCompare(UINT8 *s1,UINT8 *s2,UINT16 Len,UINT8 Mode)// 0=(s1<s2),1=(s1=s2),2=(s1>s2),Mode=1区分大小写
{
	UINT16 i;UINT8 c;
	for(i=0;i<Len;i++){c=StrCompareChar(s1[i],s2[i],Mode); if(c!=1)break;}
	return(c);
}
//-----------------------------------------------------------------------------------------------------------------------------
INT16 StrFind(UINT8 *src,UINT16 srcLen, UINT8 *std, UINT16 stdLen,UINT8 Mode)//在src中查找std,found return [0,N-1],nofound return -1
{
  ...
  {for(j=0;j<stdLen;j++){c=StrCompareChar(src[i+j],std[j],Mode);if(c!=1)break;} if(j==stdLen) return(i); i++;}
  return(-1);
}
//-----------------------------------------------------------------------------------------------------------------------------
UINT8 StrFindItem(UINT8 *src, UINT16 srcLen, UINT8 Flag,UINT16 Index,UINT16 *Pos,UINT16 *Len)//查找s中以Flag隔开的,第Index项的位置与长度
{
...
UINT8 StrFindItemStr(UINT8 *src, UINT16 srcLen, UINT8 Flag,UINT8 *std,UINT16 stdLen,UINT16 *stdIndex,UINT8 Mode)//查找s中以Flag隔开的std项
{
  UINT16 i,Len,Index=0;INT16 k[3]={0x00,0x00,0x00};UINT8 StringOn=0,ItemErr=0,ItemFirstOn=0;
	if (StrTrim(std,stdLen,&i,&Len)){std+=i;stdLen=Len-i+1;} else return(0);
  for(i=0;i<srcLen;i++)
  {
    if(src[i]=='"')
    {StringOn=!StringOn;}
    else if(!StringOn)
    {
      ...
      else if(k[0]==0x00 && k[1]==0x00 && k[2]==0x00)
      {
    			...
    			if(ItemFirstOn)
					{if(Len<stdLen){if(StrCompareChar(src[i],std[Len++],Mode)!=1)ItemErr=1;} else	{if( src[i]!=' ')ItemErr=1;}}
				}
			} 
    }
  }
...
//-----------------------------------------------------------------------------------------------------------------------------
UINT16 StrReplace(UINT8 *src, UINT16 *srcLen,UINT8 *std,UINT16 stdLen,UINT8 *srp,UINT16 srpLen,UINT8 Mode)//返回替换次数
{
  UINT16 i=0,j=0,Count=0;
  while(i+stdLen<=*srcLen)
  {
    for(j=0;j<stdLen;j++){if(StrCompareChar(src[i+j],std[j],Mode)!=1)break;}
    if(j==stdLen)
    {
      if(stdLen!=srpLen)
      {
...
{
	return(StrLTrim(src,srcLen,LPos) && StrRTrim(src,srcLen,RPos));
}
//-----------------------------------------------------------------------------------------------------------------------------
...
void StrAddItem(UINT8 *std,UINT16 *stdLen,UINT8 *src,UINT16 srcLen,UINT8 Flag)//Flag.bit0=Add ','; Flag.bit1=Add '"' ; Flag.bit2=Add 0x00 
{
  ...
  StrCat(std,stdLen,src,srcLen);if(Flag&Str_Flag_AddY)std[(*stdLen)++]='"';if(Flag&Str_Flag_AddJ)std[(*stdLen)++]=0x00;
}
//------------------------------------------------------------------------------------------------------------------------------
void StrAddItemNum(UINT8 *std,UINT16 *stdLen,double Num,UINT8 NumPCount,UINT8 Flag)//Flag.bit0=Add ','; Flag.bit1=Add '"' ; Flag.bit2=Add 0x00 
{
  ...
  NumToStr(Num,NumPCount,buf,&bufLen);StrAddItem(std,stdLen,buf,bufLen,Flag);
}
//-----------------------------------------------------------------------------------------------------------------------------
void ValueToStr(UINT8 *s,UINT64 V,UINT8 Len)
{
  if(Len>0 && Len<=8)StrCopy(s,(UINT8 *)&V,Len);
}
//-----------------------------------------------------------------------------------------------------------------------------
UINT64 StrToValue(UINT8 *s,UINT8 Len)
{
  UINT64 V=0;
  if(Len>0 && Len<=8)StrCopy((UINT8 *)&V,s,Len);return(V);
}
//-----------------------------------------------------------------------------------------------------------------------------
...
void NumToStr(double Num,UINT8 NumPCount,UINT8 *s,UINT16 *Len)
{
  ...
  while(i<PCount && Num>=(double)1.0/LdyExp10INT(PCount-i))
...
double StrToNum(UINT8 *s,UINT16 Len)
{
  ...
  {
    if(s[i]>='0' && s[i]<='9')
    {if(EXPOn){VE=VE*10+(s[i]-'0');} else if(PointOn) V=V+(s[i]-'0')/LdyExp10INT(i-pos); else V=V*10+(s[i]-'0');}
    else if(s[i]=='+'||s[i]=='-')
...
{
	return(YearDays(Year2)-YearDays(Year1));
}
//-----------------------------------------------------------------------------------------------------------------------------
UINT16 DaysBetweenMonth(INT16 Year, UINT8 Month1, UINT8 Month2)
{
  ...
  for(i=Month1;i<Month2;i++) {DBM+=DateTime_MD[i - 1];if(i == 2 && IsLeapYear(Year))DBM++;}
	return(DBM);
}
//-----------------------------------------------------------------------------------------------------------------------------
UINT8 MonthDay(INT16 Year, UINT8 Month)
{ 
  return((Month >= 1 && Month <= 12 ? DateTime_MD[Month - 1]+(Month == 2 && IsLeapYear(Year) ? 1:0):0) );	
}
//-----------------------------------------------------------------------------------------------------------------------------
UINT8 YMDToWeek(INT16 Year, UINT8 Month, UINT8 Day)
{
  ...
  for(i=1;i<Month;i++)W+=DateTime_MD[i-1]%7;W+=Day;W%=7;
  return(W);
}
//-----------------------------------------------------------------------------------------------------------------------------
...
{
  UINT32 SV=0;
  if(DT_Compare(DT,DAPP.BaseDT))
	{
    SV= DaysBetweenYear(DAPP.BaseDT.Year, DT.Year)+ (DaysBetweenMonth(DT.Year, 1, DT.Month)+DT.Day)-(DaysBetweenMonth(DAPP.BaseDT.Year,1,DAPP.BaseDT.Month)+DAPP.BaseDT.Day);
...
void UINT32ToDT(UINT32 SV,struct Data_DateTime *DT)//seconds of (Time-BaseDT)
{
  ...
  SV+=(DaysBetweenMonth(DAPP.BaseDT.Year,1,DAPP.BaseDT.Month)+DAPP.BaseDT.Day-1);
	(*DT).Year=DAPP.BaseDT.Year+SV/365;
	DBY=DaysBetweenYear(DAPP.BaseDT.Year,(*DT).Year);
  while(DBY>SV){(*DT).Year--;DBY-=YearDay((*DT).Year);}
	SV-=DBY;(*DT).Month=1+SV/29;
  DBY=DaysBetweenMonth((*DT).Year,1,(*DT).Month);
	while(DBY>SV){(*DT).Month--;DBY-=MonthDay((*DT).Year,(*DT).Month);}
	SV-=DBY;(*DT).Day=1+SV;
}
//-----------------------------------------------------------------------------------------------------------------------------
void DTToByte8(struct Data_DateTime DT,UINT8 *B,UINT8 Mode)//0=Simple,1=Struct,2=Value
{
  ...
  {
    U64.VAL=DaysBetweenYear(DAPP.BaseDT.Year, DT.Year)+(DaysBetweenMonth(DT.Year, 1, DT.Month)+DT.Day)-(DaysBetweenMonth(DAPP.BaseDT.Year,1,DAPP.BaseDT.Month)+DAPP.BaseDT.Day);
...
//-----------------------------------------------------------------------------------------------------------------------------
void Byte8ToDT(UINT8* B,struct Data_DateTime *DT,UINT8 Mode)//0=Simple,1=Struct,2=Value
{
  ...
  else if(Mode==DateTime_Mode_Value)//miniseconds of (Time-BaseDT)
  {
    ...
    U64.VAL+=(DaysBetweenMonth(DAPP.BaseDT.Year,1,DAPP.BaseDT.Month)+DAPP.BaseDT.Day-1);
	  (*DT).Year=DAPP.BaseDT.Year+U64.VAL/365;
	  DBY=DaysBetweenYear(DAPP.BaseDT.Year,(*DT).Year);
    while(DBY>U64.VAL){(*DT).Year--;DBY-=YearDay((*DT).Year);}
	  U64.VAL-=DBY;(*DT).Month=1+U64.VAL/29;
    DBY=DaysBetweenMonth((*DT).Year,1,(*DT).Month);
	  while(DBY>U64.VAL){(*DT).Month--;DBY-=MonthDay((*DT).Year,(*DT).Month);}
	  U64.VAL-=DBY;(*DT).Day=1+U64.VAL;
  }
  if(F)
  {    
	  if(B[2]>59)B[2]=59;if(B[3]>23)B[3]=23;
	  if(B[4]<1)B[4]=1;i=MonthDay(B[6]+B[7]*100,B[5]);if(B[4]>i)B[4]=i;
...
void DT_Timer(struct Data_DateTime *DT)//every 1s
{
	...
	(*DT).Day++;    if((*DT).Day<=MonthDay((*DT).Year,(*DT).Month)) return; else (*DT).Day=1;
...
float LdySin(float x)//查表
{
  ...
  if(y<0)
    ...
    {y=y-PAI/2;k++;}
  ksin=y/SinData_D;ksin=ksin%SinData_Count;
  dx=(y - (float)ksin*SinData_D);
  if ((k % 2)==0)
  {
    z=SinData[ksin]+dx*SinData[SinData_Count-ksin];
    if ((k % 4)==2) z=-z;
  }
  else
  {
    z=SinData[SinData_Count-ksin-1]-dx*SinData[ksin+1];
    if ((k % 4)==3) z=-z;
  }
  if(z<-SinData_K)
    z=-SinData_K;
...
float LdyCos(float x)//查表
{
  ...
  if ((k % 2)==0)
  {
    if (dx < 0) {ksin = ksin - 1; dx = dx + SinData_D;}
    z=SinData[SinData_Count-ksin-1]-dx*SinData[ksin+1];
    if ((k % 4)==2) z=-z;
  }
  else
...
float LdyASin(float x)//查表
{
  ...
  if (x< 0) {x = -x; Sign = 1;}
  if (x> 1) x = 1;
  M1 = 0 ; M3 = SinData_Count - 1;M2 = (M3 + M1) / 2;
  while (Found==0)
  {
    if ( M2 == M1 || M2 == M3 )
      Found = 1;
    else if (SinData_K*x > SinData[M2])
      {M1 = M2; M2 = (M3 + M2) / 2;}
    else if (SinData_K*x < SinData[M2])
      {M3 = M2; M2 = (M2 + M1) / 2;}
    else
  ...
  z =M2 * SinData_D;
  if (M2 < SinData_Count - 1) z = z + (SinData_K*x - SinData[M2]) / SinData[SinData_Count - M2 - 1];
  if (Sign) z = -z;
  return (z);
}
//------------------------------------------------------------------------------------------------------------------------------
float LdyACos(float x)//查表
{
  int M1, M2, M3;
  int Sign=0, Found=0;
  float z;
  ...
  z = (SinData_Count-1-M2) * SinData_D;
  if (M2 > 0) z = z + (SinData_K*x - SinData[SinData_Count - M2 - 1]) / SinData[M2];
  if (Sign) z = (float)PAI-z;
  return (z);
}
//------------------------------------------------------------------------------------------------------------------------------
float LdyTan(float x)//查表
{
  float y1,y2;
  y1=LdySin(x);y2=LdyCos(x);
  if(y2==0)
...
float LdyATan(float x)//查表
{
  ...
  while (Found==0)
  {
    y1= SinData[M2];y2=SinData[SinData_Count-M2-1];
    if ( M2 == M1 || M2 == M3 )
      Found = 1;
    else if (y2*x >y1)
      {M1 = M2; M2 = (M3 + M2) / 2;}
...
float LdySqrt(float x)//查表
{
  ...
  k=x/SqrtData_D;x0=SqrtData_D*k;d=x-x0;y0=(float)SqrtData[k]/SqrtData_K;
...
double LdyExpINT(int k)
{
  ...
  if(k>=100){z*=Double_EXPE[18+k/100-1];k%=100;}
  if(k>=10){z*=Double_EXPE[9+k/10-1];k%=10;}
  if(k>=1)z*=Double_EXPE[k-1];
...
double LdyExp10INT(int k)
{
  ...
  if(k>=100){z*=Double_EXPT[18+k/100-1];k%=100;}
  if(k>=10){z*=Double_EXPT[9+k/10-1];k%=10;}
  if(k>=1)z*=Double_EXPT[k-1];
  return(Sign ? (double)1.0/z:z);
}
#endif
//------------------------------------------------------------------------------------------------------------------------------
#if LdyEXPLOG_On
double LdyExp(double x)//泰勒级数+查表
{
  ...
  z*=LdyExpINT(k1); if(Sign)z=(double)1.0/z;
  return(z);  
}
//------------------------------------------------------------------------------------------------------------------------------
double LdyLog(double x)//泰勒级数
{
...
{
  return(LdyExp(x*(double)2.3025850929940456840179914546844));//ln(10)
}
//------------------------------------------------------------------------------------------------------------------------------
double LdyLog10(double x)
{
  return(LdyLog(x)/(double)2.3025850929940456840179914546844);//ln(10)
}
//------------------------------------------------------------------------------------------------------------------------------
double LdyPow(double x,double y)
{
  ...
  if(y==ky)
  {
    ...
    if(x==10)
    {z=LdyExp10INT(ky);Found=1;}
    else if(x==EXP)
    {z=LdyExpINT(ky);Found=1;}
    else if(ky<10)
    {for(i=0;i<ky;i++)z*=x;Found=1;}
    if(Found){if(SignExp) z=1/z;if(Sign)z=-z; return(z);}
  }
  z=LdyExp(y*LdyLog(x)); if(Sign)z=-z;
  return(z);
}
#endif
//==============================================================================================================================
