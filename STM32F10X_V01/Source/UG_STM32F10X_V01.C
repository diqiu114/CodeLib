...
//==============================================================================================================================
//const
UINT32 const            UG_BASE_ADC[]={BASE_ADC1,BASE_ADC2,BASE_ADC3};
UINT32 const            UG_BASE_SPI[]={BASE_SPI1,BASE_SPI2,BASE_SPI3};
UINT32 const            UG_BASE_TIM[]={BASE_TIM1,BASE_TIM2,BASE_TIM3,BASE_TIM4,BASE_TIM5,BASE_TIM6,BASE_TIM7,BASE_TIM8,BASE_TIM9,\
                        BASE_TIM10,BASE_TIM11,BASE_TIM12,BASE_TIM13,BASE_TIM14,BASE_TIM15,BASE_TIM16,BASE_TIM17};
UINT32 const            UG_BASE_UART[]={BASE_UART1,BASE_UART2,BASE_UART3,BASE_UART4,BASE_UART5};
//------------------------------------------------------------------------------------------------------------------------------
const UINT32 UG_DTF_P_Count=12,UG_DTF_P[32][2]={
                {(UINT32)&UG_Init_SFR,7},{(UINT32)&UG_Init_RCC,89},{(UINT32)&UG_Init_SYS,101},{(UINT32)&UG_Init_FLASH,11},  
                {(UINT32)&UG_Init_GPIO,59},{(UINT32)&UG_Init_EXTI,4},{(UINT32)&UG_Init_SYSTICK,6},{(UINT32)&UG_Init_RTC,4},
                {(UINT32)&UG_Init_TIM,179},{(UINT32)&UG_Init_ADC,85},{(UINT32)&UG_Init_UART,117},{(UINT32)&UG_Init_MEM,134},
                };
//==============================================================================================================================
int UG_main(void)
{
...
void UG_Init_SPI(void)
{
  ...
  for(Var[0]=0;Var[0]<SDK_SPI_COUNT;Var[0]++)
  {
    ...
    SPI_I2SPR(Var[2])=0X0000;
	  ...
	  if(!DSPI.Item[Var[0]].Setup.EN)UG_SPI_SetClock(Var[2],0);
  }
  #endif
}
//------------------------------------------------------------------------------------------------------------------------------
...
void UG_Init_UART(void)
{
  ...
  for(Var[0]=0;Var[0]<SDK_UART_COUNT;Var[0]++)
  {
    ...
    UG_Uart_SetClock(Var[1],1);
    UG_Uart_SetBaud(Var[1]);
...
void UG_Init_CAN(void)
{
  ...
  for(Var[0]=0;Var[0]<SDK_CAN_COUNT;Var[0]++)
  {
    ...
    CAN_FR1(Var[2],0)=UG_CAN_GetID(Var[2],1,0);
    CAN_FR2(Var[2],0)=0xFFFFFFF0;
    CAN_FR1(Var[2],1)=UG_CAN_GetID(Var[2],1,1);
...
void UG_Init_MEM(UINT8 IsAfter)
{
  ...
  if(!IsAfter)
  {
	  for(Var[0]=0;Var[0]<3;Var[0]++){ADC[Var[0]]=(Type_ADC *)UG_BASE_ADC[Var[0]];SPI[Var[0]]=(Type_SPI *)UG_BASE_SPI[Var[0]];}
    for(Var[0]=0;Var[0]<17;Var[0]++)TIM[Var[0]]=(Type_TIM *)UG_BASE_TIM[Var[0]];
	  for(Var[0]=0;Var[0]<5;Var[0]++)UART[Var[0]]=(Type_UART *)UG_BASE_UART[Var[0]];
  ...
  else
  {
    ...
    UG_Byte8ToDT((UINT8 *)Device_BaseDT,&DAPP.BaseDT,0);UG_Byte8ToDT((UINT8 *)Device_MadeDT,&DAPP.MadeDT,0);UG_StrCopy((UINT8*)&DAPP.DT,(UINT8 *)&DAPP.MadeDT,8);
    ...
    DDEV.Item[0].NSN.Item=DDEV.Item[0].NSNBUF; UG_StrCopy(DDEV.Item[0].NSN.Item,(UINT8 *)((UINT32)&Device_NSN[1]),DDEV.Item[0].NSN.Count);
    ...
    for(Var[0]=0;Var[0]<SDK_UART_COUNT;Var[0]++)  
    {
      ...
      DCOM.Item[SDK_UART_Index+Var[0]].PRTC.Index=0;UG_COM_PRTC_Init(SDK_UART_Index+Var[0]);
    }
    #endif
    #if SDK_CAN_COUNT
    for(Var[0]=0;Var[0]<SDK_CAN_COUNT;Var[0]++)  
    ...
    #endif
    #endif
    //if(!Flash_Dentify(Dentify)){while(1);}
  }
...
{while(1);}	
//------------------------------------------------------------------------------------------------------------------------------
void UG_SysTick_Handler(void)//1ms
{
...
//==============================================================================================================================
void UG_WWDG_IRQHandler(void)
{while(1);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_PVD_IRQHandler(void)
{while(1);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_TAMPER_IRQHandler(void)
{while(1);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_RTC_IRQHandler(void)
{while(1);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_FLASH_IRQHandler(void)
{while(1);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_RCC_IRQHandler(void)
{while(1);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_EXTI0_IRQHandler(void)
{
	if(EXTI.IMR_BIT.B0 && EXTI.PR_BIT.B0)	UG_EXTI_IRQ_Control(0);
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_EXTI1_IRQHandler(void)
{
	if(EXTI.IMR_BIT.B1 && EXTI.PR_BIT.B1)	UG_EXTI_IRQ_Control(1);
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_EXTI2_IRQHandler(void)
{
	if(EXTI.IMR_BIT.B2 && EXTI.PR_BIT.B2)	UG_EXTI_IRQ_Control(2);
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_EXTI3_IRQHandler(void)
{
	if(EXTI.IMR_BIT.B3 && EXTI.PR_BIT.B3)	UG_EXTI_IRQ_Control(3);
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_EXTI4_IRQHandler(void)
{
	if(EXTI.IMR_BIT.B4 && EXTI.PR_BIT.B4)	UG_EXTI_IRQ_Control(4);
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_DMA1_Channel1_IRQHandler(void)
{UG_DMA_IRQ_Control(0,0);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_DMA1_Channel2_IRQHandler(void)
{UG_DMA_IRQ_Control(0,1);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_DMA1_Channel3_IRQHandler(void)
{UG_DMA_IRQ_Control(0,2);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_DMA1_Channel4_IRQHandler(void)
{UG_DMA_IRQ_Control(0,3);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_DMA1_Channel5_IRQHandler(void)
{UG_DMA_IRQ_Control(0,4);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_DMA1_Channel6_IRQHandler(void)
{UG_DMA_IRQ_Control(0,5);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_DMA1_Channel7_IRQHandler(void)
{UG_DMA_IRQ_Control(0,6);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_ADC12_IRQHandler(void)// ADC12
{
	if((*ADC[0]).SR_BIT.EOC){(*ADC[0]).SR&=0xFFF8;UG_ADC_IRQ_Control(0);} //AWD=EOC=JEOC=0
	if((*ADC[1]).SR_BIT.EOC){(*ADC[1]).SR&=0xFFF8;UG_ADC_IRQ_Control(1);} //AWD=EOC=JEOC=0
}
//------------------------------------------------------------------------------------------------------------------------------
...
{
  if(EXTI.IMR_BIT.B5 && EXTI.PR_BIT.B5)	UG_EXTI_IRQ_Control(5);
  if(EXTI.IMR_BIT.B6 && EXTI.PR_BIT.B6)	UG_EXTI_IRQ_Control(6);
  if(EXTI.IMR_BIT.B7 && EXTI.PR_BIT.B7)	UG_EXTI_IRQ_Control(7);
  if(EXTI.IMR_BIT.B8 && EXTI.PR_BIT.B8)	UG_EXTI_IRQ_Control(8);
  if(EXTI.IMR_BIT.B9 && EXTI.PR_BIT.B9)	UG_EXTI_IRQ_Control(9);
}
//------------------------------------------------------------------------------------------------------------------------------
...
{while(1);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_TIM2_IRQHandler(void)//TIM2,100us
...
{UG_I2C_Event_Control(0);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_I2C1_ER_IRQHandler(void)//I2C1 Error
{UG_I2C_Error_Control(0);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_I2C2_EV_IRQHandler(void)//I2C2 Event
{UG_I2C_Event_Control(1);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_I2C2_ER_IRQHandler(void)//I2C2 Error
{UG_I2C_Error_Control(1);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_SPI1_IRQHandler(void)//SPI1
{UG_SPI_IRQ_Control(0);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_SPI2_IRQHandler(void)//SPI2
{UG_SPI_IRQ_Control(1);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_USART1_IRQHandler(void)//USART1
{UG_Uart_IRQ_Control(0);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_USART2_IRQHandler(void)//USART2
{UG_Uart_IRQ_Control(1);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_USART3_IRQHandler(void)//USART3
{UG_Uart_IRQ_Control(2);}
//------------------------------------------------------------------------------------------------------------------------------
void UG_EXTI15_10_IRQHandler(void)//EXTI Line 15..10
{
	if(EXTI.IMR_BIT.B10 && EXTI.PR_BIT.B10)	UG_EXTI_IRQ_Control(10);
	if(EXTI.IMR_BIT.B11 && EXTI.PR_BIT.B11)	UG_EXTI_IRQ_Control(11);
	if(EXTI.IMR_BIT.B12 && EXTI.PR_BIT.B12)	UG_EXTI_IRQ_Control(12);
	if(EXTI.IMR_BIT.B13 && EXTI.PR_BIT.B13)	UG_EXTI_IRQ_Control(13);
	if(EXTI.IMR_BIT.B14 && EXTI.PR_BIT.B14)	UG_EXTI_IRQ_Control(14);
	if(EXTI.IMR_BIT.B15 && EXTI.PR_BIT.B15)	UG_EXTI_IRQ_Control(15);
}
//------------------------------------------------------------------------------------------------------------------------------
...
{
#if SDK_UART_COUNT  
  if((*UART[Index]).SR_BIT.ORE){UG_Uart_Reset(Index);return;}
	if((*UART[Index]).SR_BIT.RXNE)UG_Uart_Read(Index);
  if((*UART[Index]).SR_BIT.TC)UG_Uart_Write(Index);
#endif  
}
//------------------------------------------------------------------------------------------------------------------------------
#if SDK_UART_COUNT
...
{
  UG_Uart_SetClock(Index,0);NOP();NOP();NOP();UG_Uart_SetClock(Index,1);
  UART_CR1(Index)=0x0000002C; UART_SR(Index)=0x00000000;
  UG_Uart_EndSendItem(Index);if(DCOM.Item[DUART.COMIndex[Index]].TF.Count)UG_Uart_StartSendItem(Index);
}
//------------------------------------------------------------------------------------------------------------------------------
...
void UG_Uart_Write(UINT16 Index)
{
  ...
  if(DCOM.Item[COMIndex].TF.Count==0x00||!COM_TX_On(COMIndex)){COM_TX_On(COMIndex)=0;UG_Uart_EndSendItem(Index);return;}
  UG_Uart_SendBUF(Index);
}
//-----------------------------------------------------------------------------------------------------------------------------
void UG_Uart_SendBUF(UINT16 Index)
{
  ...
  else
  {
    if((*TF).MMT.Count>1 && j==(*TF).MMT.Item[1].Pos)
    {
      ...
      if((*TF).MMT.Item[0].Time){UG_COM_EndSend(COMIndex);return;}
    }
    F1=1;
  }
  if(F1)
...
{
  if(!COM_TXItem_On(DUART.COMIndex[Index])){(*UART[Index]).SR_BIT.TC=0;(*UART[Index]).CR1_BIT.TCIE=1;UG_Uart_SendBUF(Index);}
}
//-----------------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------------------
void UG_Uart_WaitSend(UINT16 Index)//等待缓冲发送完毕(中断外调用)
{
  UINT8 COMIndex;
...
void UG_CAN_Write(UINT16 Index)
{
  ...
  COMIndex=DCAN.COMIndex[Index];
	...
	if(DCOM.Item[COMIndex].TF.Count==0x00||!COM_TX_On(COMIndex)){COM_TX_On(COMIndex)=0;UG_CAN_EndSendItem(Index);return;}
	UG_CAN_SendBUF(Index);
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_CAN_SendBUF(UINT16 Index)
{
  ...
  if(F0)
  {
    ...
    if((*TF).Count==0x0000)(*TF).MMT.Count=0x00;
	  ...
	  CAN[Index].TXM[FIFOIndex].TIR=UG_CAN_GetID(Index,0,0);
...
{
  if(!COM_TXItem_On(DCAN.COMIndex[Index]))UG_CAN_SendBUF(Index);
}
//------------------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------------------
void UG_CAN_WaitSend(UINT16 Index)
{
  while(DCOM.Item[DCAN.COMIndex[Index]].TF.Count||COM_TXItem_On(DCAN.COMIndex[Index]));
}
//------------------------------------------------------------------------------------------------------------------------------
UINT32 UG_CAN_GetID(UINT16 Index,UINT8 IsFilter,UINT8 IsBroadcast)
{
	UINT32 i;
...
{
	USB.CH[EPIndex].RXCOUNT=UG_USB_CountToRXCount(Count);
}
//-----------------------------------------------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------------------------------------------
void UG_USB_SetDeviceAddress(UINT8 DEVAddr)//0x00_0x7F
{
...
{
  UINT32 i;
  for (i = 0; i < USB_EPCount; i++)UG_USB_SetEPAddress(i,i);UG_USB_SetDeviceAddress(DEVAddr);
}
//-----------------------------------------------------------------------------------------------------------------------------
...
{
  COM_TX_On(Index)=1;COM_TXItem_On(Index)=0;UG_COM_StartSendItem(Index);
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_COM_EndSend(UINT16 Index)
{
  COM_TX_On(Index)=0;UG_COM_EndSendItem(Index);
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_COM_StartSendItem(UINT16 Index)//针对中断式发送(因为立即与定时都有各自进程)
{
  #if SDK_UART_COUNT
  if(Index>=DUART.Index && Index<DUART.Index+DUART.Count){UG_Uart_StartSendItem(DUART.Item[Index-DUART.Index].HWIndex);return;}
  #endif
  #if SDK_CAN_COUNT
  if(Index>=DCAN.Index && Index<DCAN.Index+DCAN.Count){UG_CAN_StartSendItem(DCAN.Item[Index-SDK_CAN_Index].HWIndex);return;}
  #endif
}
//------------------------------------------------------------------------------------------------------------------------------
...
{
  #if SDK_UART_COUNT
  if(Index>=DUART.Index && Index<DUART.Index+DUART.Count){UG_Uart_EndSendItem(DUART.Item[Index-DUART.Index].HWIndex);return;}
  #endif
  #if SDK_CAN_COUNT
  if(Index>=DCAN.Index && Index<DCAN.Index+DCAN.Count){UG_CAN_EndSendItem(DCAN.Item[Index-DCAN.Index].HWIndex);return;}
  #endif
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_COM_SendString(UINT16 Index,UINT8 *s,UINT16 Count,UINT32 Time)//Uart+Can+USB,Time=延迟发送时间
{
  UINT16 i,j,k,k1,k2,Pos;  struct Data_COM_TXFrame *TF;
  if(Count==0||DCOM.Item[Index].TF.Count+Count+2>DCOM.Item[Index].TF.Size){COM_TXItem_On(Index)=0;UG_COM_StartSendItem(Index); return;}//因为不知明原因硬件发送被挂起，需要重新触发
  ...
  if(!COM_TXContinue_On(Index)){if(Time){COM_MMTTime_On=1;} else if(!COM_TX_On(Index)){UG_COM_StartSend(Index);}}
}
//------------------------------------------------------------------------------------------------------------------------------
...
{ 
	...
	while(*Data!=0)//判断是否到达字符串结束符
  {
		if(*Data=='\\')// '\'=0x5c
    {
      ...
      {
				case 'r'://回车符
					buf[0]=0x0d;UG_COM_SendString(Index, buf,1,Time);	   
					break;
				case 'n':							          //换行符
					buf[0]=0x0a;UG_COM_SendString(Index,buf,1,Time);
					break;
				default:
				  break;
			} 		 
		...
		else if(*Data=='%')
    {
      ...
      {				
				case 's'://字符串
         	Found=1;s = va_arg(ap, const UINT8 *);Len=UG_StrLen((UINT8 *)s,255);
          UG_COM_SendString(Index,(UINT8 *)s,Len,Time);
          break;
        case 'd'://十进制
        case 'f':
          Found=1;PCount=((*Data)=='d' ? 0:7);d = va_arg(ap,double); UG_NumToStr(d,PCount, buf, &Len);
          UG_COM_SendString(Index,buf,Len,Time);
        case 'l':
          if(*(Data+1)=='d'||*(Data+1)=='f')
          {
            Found=1;PCount=((*Data)=='d' ? 0:15);d = va_arg(ap,double); UG_NumToStr(d,PCount, buf, &Len);
            UG_COM_SendString(Index,buf,Len,Time);
          }
         	break;
				default:
      ...
      if(!Found){buf[0]='%';buf[1]=*(Data);UG_COM_SendString(Index,buf,2,Time);}
		}
		else
    {buf[0]=*Data;UG_COM_SendString(Index,buf,1,Time);}
...
void UG_COM_Send_Timer(void)//Every 100us
{
  ...
  for(i=0;i<SDK_COM_COUNT;i++)
  {
    ...
    if(IRQTempFlag1)UG_COM_StartSend(i);
  }
...
{
  DCOM.Item[Index].PRTC.TimeIndex=UG_COM_PRTC_CodeToIndex(DCOM.Item[Index].PRTC.Item[DCOM.Item[Index].PRTC.Index]);
  if(Index==0)
  {
...
void UG_COM_PRTC_Timer(void)//every 100ms
{
  ...
  for(i=0;i<SDK_COM_COUNT;i++)
  {
    ...
    if(DCOM.Item[i].PRTC.TimeCount>=j && DCOM.Item[i].TF.Count==0x0000) 
    {
      ...
      {DCOM.Item[i].PRTC.Index=(DCOM.Item[i].PRTC.Index+1)%DCOM.Item[i].PRTC.Count;UG_COM_PRTC_Init(i);}
    }
  }
}
//------------------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------------------
void UG_GPIO_SetMultiPinMode(UINT8 GPIOIndex,UINT32 PIN,UINT8 Mode)
{
...
{ 
  ...
  do{status =UG_Flash_GetStatus(IsBank2);Timeout--;} while((status == FLASH_BUSY) && Timeout );
...
{
  Flash_Status status = FLASH_COMPLETE;
  status = UG_Flash_WaitForLastOperation(IsBank2,FLASH_EraseTimeout);
...
Flash_Status UG_Flash_EnableWriteProtection(UINT8 IsBank2,UINT32 FLASH_Pages)
{
  ...
  WRP.VAL=~FLASH_Pages;
  status = UG_Flash_WaitForLastOperation(IsBank2,FLASH_ProgramTimeout);
  if(status != FLASH_COMPLETE)return(status);
  
  FLASH.OPTKEYR = FLASH_KEY1; FLASH.OPTKEYR = FLASH_KEY2; FLASH.CR_BIT.OPTPG=1;
  for(i=0;i<4;i++)
  {if((status == FLASH_COMPLETE) && (WRP.B[i] != 0xFF)) {OB.WRP[i] = WRP.B[i];status = UG_Flash_WaitForLastOperation(IsBank2,FLASH_ProgramTimeout);}}
...
Flash_Status UG_Flash_EraseAddress(UINT32 Address)
{
  ...
  {
    status = UG_Flash_WaitForLastOperation(0,FLASH_EraseTimeout);//Wait for last operation to be completed
    if(status == FLASH_COMPLETE)
    { 
...
{
  return (UG_Flash_EraseAddress(BASE_FLASH+(Page&0x7F)*SIZE_FLASHPAGE)); 
}
//-----------------------------------------------------------------------------------------------------------------------------
...
Flash_Status UG_Flash_EraseOptionBytes(UINT8 IsBank2)
{
  ...
  if(status == FLASH_COMPLETE)
  {
    ...
    status = UG_Flash_WaitForLastOperation(IsBank2,FLASH_EraseTimeout);//Wait for last operation to be completed
    if(status == FLASH_COMPLETE)
    {
      ...
      status = UG_Flash_WaitForLastOperation(IsBank2,FLASH_ProgramTimeout);//Wait for last operation to be completed
...
Flash_Status UG_Flash_UserOptionByteConfig(UINT16 OB_IWDG,UINT16 OB_STOP,UINT16 OB_STDBY)
{
  ...
  FLASH.OPTKEYR = FLASH_KEY1; FLASH.OPTKEYR = FLASH_KEY2;//Authorize the small information block programming
  status = UG_Flash_WaitForLastOperation(0,FLASH_ProgramTimeout);
  if(status == FLASH_COMPLETE)
  {  
...
//-----------------------------------------------------------------------------------------------------------------------------
UINT16 UG_Flash_WriteString(UINT32 Address,UINT8 *s,UINT16 Len)//返回剩余字节数
{
  ...
  while(Len)
  {
    ...
    {
      if(SubPos==0)
      {if(UG_Flash_EraseAddress(Address)!=FLASH_COMPLETE)break;}
      else
      {if(UG_Flash_WaitForLastOperation(0,FLASH_ProgramTimeout)!= FLASH_COMPLETE)break;}
      FLASH.CR_BIT.PG=1;
...
{
  UG_Flash_UnLock();UG_Flash_WriteString(Address,s,Len);UG_Flash_Lock();
}
//-----------------------------------------------------------------------------------------------------------------------------
UINT8 UG_Flash_IsNew(void)
{
  if(UG_StrCompare((UINT8 *)Device_FirstRun,(UINT8 *)DC_FirstRun,UG_StrLen((UINT8 *)DC_FirstRun,64),0)!=1)//首次运行
  {UG_Flash_UnLock();UG_Flash_WriteString((UINT32)Device_FirstRun,(UINT8 *)DC_FirstRun, UG_StrLen((UINT8 *)DC_FirstRun,64));UG_Flash_Lock();return(1);}  
  return(0);
}
//==============================================================================================================================
UINT8 UG_Flash_Dentify(UINT64 DV)
{
  ...
  for(i=0;i<UG_DTF_P_Count;i++)
  {for(j=0;j<UG_DTF_P[i][1]*4;j++){UG_Flash_ReadString(UG_DTF_P[i][0]-1+j,&c,1);DAPP.TBUF[j&7]^=c;}}
  i=0;for(j=0;j<8;j++){DAPP.TBUF[i]^=(Device_BaseDT[j]);i=(i+1)%8;};
  i=0;for(j=0;j<8;j++){DAPP.TBUF[i]^=(Device_MadeDT[j]);i=(i+1)%8;};
  i=0;for(j=0;j<24;j++){DAPP.TBUF[i]^=(Device_ID[j]);i=(i+1)%8;};
  i=0;for(j=0;j<UG_StrLen((UINT8 *)Device_CP,0x40);j++){DAPP.TBUF[i]^=(Device_CP[j]+3);i=(i+1)%8;};
  i=0;for(j=0;j<UG_StrLen((UINT8 *)Device_NV,0x40);j++){DAPP.TBUF[i]^=(Device_NV[j]+3);i=(i+1)%8;};
