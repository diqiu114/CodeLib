...
#if SDK_USB_COUNT
//------------------------------------------------------------------------------------------------------------------------------
const UINT8 UG_CustomHID_DeviceDescriptor[CUSTOMHID_SIZ_DEVICE_DESC]=
        {
        0x12,                       //bLength
...
const UINT8 UG_CustomHID_ConfigDescriptor[CUSTOMHID_SIZ_CONFIG_DESC] =//Len=0x29,All Descriptors (Configuration, Interface, Endpoint, Class, Vendor
        {
...
const UINT8 UG_CustomHID_ReportDescriptor[CUSTOMHID_SIZ_REPORT_DESC] = //Len=0x21
        { 
...
const UINT8 UG_CustomHID_StringLangID[CUSTOMHID_SIZ_STRING_LANGID] =//Len=0x04
        {CUSTOMHID_SIZ_STRING_LANGID, USB_STRING_DESCRIPTOR, 0x09, 0x04};

//制造商字符串,Manufacturer: "www.waveshare.net"
const UINT8 UG_CustomHID_StringVendor[CUSTOMHID_SIZ_STRING_VENDOR] =//Len=0x28
        {
...
//产品名称
const UINT8 UG_CustomHID_StringProduct[CUSTOMHID_SIZ_STRING_PRODUCT] =//Len=0x20
       {
...
//产品名称
UINT8 UG_CustomHID_StringSerial[CUSTOMHID_SIZ_STRING_SERIAL] =//Len=0x1A
        {CUSTOMHID_SIZ_STRING_SERIAL,USB_STRING_DESCRIPTOR,'A', 0, 'P', 0, 'M', 0,'3', 0,'2', 0, '1', 0, '0', 0 };
//------------------------------------------------------------------------------------------------------------------------------
ONE_DESCRIPTOR UG_Device_Descriptor ={(UINT8*)UG_CustomHID_DeviceDescriptor, CUSTOMHID_SIZ_DEVICE_DESC };
ONE_DESCRIPTOR UG_Config_Descriptor ={(UINT8*)UG_CustomHID_ConfigDescriptor, CUSTOMHID_SIZ_CONFIG_DESC};
ONE_DESCRIPTOR UG_CustomHID_Report_Descriptor ={(UINT8 *)UG_CustomHID_ReportDescriptor, CUSTOMHID_SIZ_REPORT_DESC};
ONE_DESCRIPTOR UG_CustomHID_Hid_Descriptor ={(UINT8*)UG_CustomHID_ConfigDescriptor + CUSTOMHID_OFF_HID_DESC,CUSTOMHID_SIZ_HID_DESC};
ONE_DESCRIPTOR UG_String_Descriptor[4] =
        {
          {(UINT8*)UG_CustomHID_StringLangID, CUSTOMHID_SIZ_STRING_LANGID},{(UINT8*)UG_CustomHID_StringVendor, CUSTOMHID_SIZ_STRING_VENDOR},
          {(UINT8*)UG_CustomHID_StringProduct, CUSTOMHID_SIZ_STRING_PRODUCT},{(UINT8*)UG_CustomHID_StringSerial, CUSTOMHID_SIZ_STRING_SERIAL}
        };
//------------------------------------------------------------------------------------------------------------------------------
...
USB_RESULT UG_Standard_SetInterface(void)
{
  ...
  Re =UG_CustomHID_Get_Interface_Setting(DUSB.Device_Info.USBwIndexs.LH.L,DUSB.Device_Info.USBwValues.LH.L);
  if(DUSB.Device_Info.Current_Configuration != 0)
  {
    ...
    {UG_USB_NOP_Process();DUSB.Device_Info.Current_Interface = DUSB.Device_Info.USBwIndexs.LH.L;DUSB.Device_Info.Current_AlternateSetting = DUSB.Device_Info.USBwValues.LH.L; return USB_SUCCESS;}
  }
  return USB_UNSUPPORT;
}
//------------------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------------------
USB_RESULT UG_Standard_ClearFeature(void)//Clear or disable a specific feature.
{
  if (USB_Type_Recipient == (USB_STANDARD_REQUEST | USB_DEVICE_RECIPIENT))
  ...
  else if (USB_Type_Recipient == (USB_STANDARD_REQUEST | USB_ENDPOINT_RECIPIENT))
  {
    ...
    if ( DUSB.Device_Info.USBwIndexs.LH.L & 0x80)
    {
      ...
      {
				if(USB.EPR_BIT[DUSB.REP].TXDTOG)UG_USB_ToggleDTOG(DUSB.REP,1);
				UG_USB_SetTXSTAT(DUSB.REP,USB_EPR_STAT_VALID); 
      }
    }
    else
    {
      ...
      {
        if (DUSB.REP == 0)
        {UG_USB_SetRXCount(0,DUSB.EPSIZE[0]);}//After clear the STALL, enable the default endpoint receiver
        else
        {if(USB.EPR_BIT[DUSB.REP].RXDTOG)UG_USB_ToggleDTOG(DUSB.REP,0);}
				UG_USB_SetRXSTAT(DUSB.REP,USB_EPR_STAT_VALID); 
      }
    }
    UG_USB_NOP_Process();
    return USB_SUCCESS;
...
//------------------------------------------------------------------------------------------------------------------------------
USB_RESULT UG_Standard_SetEndPointFeature(void)//Set or enable a specific feature of EndPoint
{
  DUSB.REP = DUSB.Device_Info.USBwIndexs.LH.L & 0x7F;
  ...
  {if (DUSB.Device_Info.USBwIndexs.LH.L & 0x80){UG_USB_SetTXSTAT(DUSB.REP,USB_EPR_STAT_STALL);} else {UG_USB_SetRXSTAT(DUSB.REP,USB_EPR_STAT_STALL);}}
  UG_USB_NOP_Process();
  return USB_SUCCESS;
}
//------------------------------------------------------------------------------------------------------------------------------
...
{
  DUSB.Device_Info.Current_Feature|=0x20; UG_USB_NOP_Process();
  return USB_SUCCESS;
}
//------------------------------------------------------------------------------------------------------------------------------
...
void UG_CustomHID_Init(void)
{
  ...
  UG_USB_Get_SerialNum();//Update the serial number string descriptor with the data from the unique ID
  UG_USB_PowerOn();//Connect the device
...
void UG_CustomHID_Reset(void)
{
  ...
  DUSB.Device_Info.Current_Feature = UG_CustomHID_ConfigDescriptor[7];//Current Feature initialization
  USB_BUFTAB=USB_BUFTAB_Value&0xFFF8;
	//Initialize Endpoint 0
	UG_USB_SetEPType(0,USB_EPR_TYPE_CONTROL);
	UG_USB_SetTXSTAT(0,USB_EPR_STAT_STALL);
	USB.CH[0].RXADDR=(DUSB.RXADDR[0]-BASE_USBRAM)>>1;
	USB.CH[0].TXADDR=(DUSB.TXADDR[0]-BASE_USBRAM)>>1;
	UG_USB_Clear_KIND(0);
	UG_USB_SetRXCount(0,DUSB.EPSIZE[0]);
	UG_USB_SetRXSTAT(0,USB_EPR_STAT_VALID);
  //Initialize Endpoint 1 从机收
	UG_USB_SetEPType(1,USB_EPR_TYPE_BULK);
  USB.CH[1].RXADDR =(DUSB.RXADDR[1]-BASE_USBRAM)/2;
	UG_USB_SetRXCount(1,DUSB.EPSIZE[1]);//这里数据长度不能设为变量，只能是常量
	UG_USB_SetRXSTAT(1, USB_EPR_STAT_VALID);
  //Initialize Endpoint 2 从机发
  UG_USB_SetEPType(2,USB_EPR_TYPE_BULK);
  USB.CH[2].TXADDR=(DUSB.TXADDR[2]-BASE_USBRAM)/2;
	USB.CH[2].TXCOUNT=DUSB.EPSIZE[2];//就是因为在这里设置了，使得每次数据长度都不能超过设置的数
  UG_USB_SetTXSTAT(2,USB_EPR_STAT_DIS);
	UG_USB_SetTXSTAT(2,USB_EPR_STAT_NAK);
...
{}
//------------------------------------------------------------------------------------------------------------------------------
USB_RESULT UG_CustomHID_Data_Setup(UINT8 RequestNo)
{
...
//------------------------------------------------------------------------------------------------------------------------------
USB_RESULT UG_CustomHID_NoData_Setup(UINT8 RequestNo)
{
  return ((USB_Type_Recipient == (USB_CLASS_REQUEST | USB_INTERFACE_RECIPIENT))  && (RequestNo == USB_SET_PROTOCOL) ? UG_CustomHID_SetProtocol():USB_UNSUPPORT);
}
//------------------------------------------------------------------------------------------------------------------------------
UINT8 *UG_CustomHID_GetDeviceDescriptor(UINT16 Length)
{
  return UG_Standard_GetDescriptorData(Length,&UG_Device_Descriptor); 
}
//------------------------------------------------------------------------------------------------------------------------------
UINT8 *UG_CustomHID_GetConfigDescriptor(UINT16 Length)
{
  return UG_Standard_GetDescriptorData(Length, &UG_Config_Descriptor);
}
//------------------------------------------------------------------------------------------------------------------------------
UINT8 *UG_CustomHID_GetStringDescriptor(UINT16 Length)
{
  return (DUSB.Device_Info.USBwValues.LH.L > 4 ? NULL:UG_Standard_GetDescriptorData(Length, &UG_String_Descriptor[DUSB.Device_Info.USBwValues.LH.L]));
}
//------------------------------------------------------------------------------------------------------------------------------
UINT8 *UG_CustomHID_GetReportDescriptor(UINT16 Length)
{
  return UG_Standard_GetDescriptorData(Length, &UG_CustomHID_Report_Descriptor);
}
//------------------------------------------------------------------------------------------------------------------------------
UINT8 *UG_CustomHID_GetHIDDescriptor(UINT16 Length)
{
  return UG_Standard_GetDescriptorData(Length, &UG_CustomHID_Hid_Descriptor);
}
//------------------------------------------------------------------------------------------------------------------------------
...
void UG_USB_Get_SerialNum(void)
{
  ...
  UG_USB_IntToUnicode (DES.UID[0]+DES.UID[2], &UG_CustomHID_StringSerial[2] , 8);
  UG_USB_IntToUnicode (DES.UID[1], &UG_CustomHID_StringSerial[18], 4);
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_USB_DataStageOut(void)
{
  ...
  if (DUSB.Device_Info.Ctrl_Info.CopyData && DUSB.Device_Info.Ctrl_Info.Usb_wLength)
  {
    ...
    UG_USB_GetData(0,Buffer,DUSB.Device_Info.Ctrl_Info.PacketSize);
  }
  ...
  else
  {
    ...
    else if (DUSB.Device_Info.Ctrl_Info.Usb_wLength == 0)
    {
      ...
      UG_USB_SendZeroLength();
    }
  }
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_USB_DataStageIn(void)	 //这个函数完成描述符的输出准备
{
  UINT8 *DataBuffer;
  UINT32 Length;
  ...
  if ((DUSB.Device_Info.Ctrl_Info.Usb_wLength == 0) && (DUSB.Device_Info.ControlState == LAST_IN_DATA))
  {
    ...
    {UG_USB_SendZeroLength(); DUSB.Device_Info.ControlState = LAST_IN_DATA;USB_MulMaxPacketSize = 0;}
  ...
  DataBuffer = (*DUSB.Device_Info.Ctrl_Info.CopyData)(Length);	 //这个是用户取得用户描述符缓冲区的地址，这里共有18字节
	UG_USB_SetData(0,DataBuffer,Length);//这个函数将设备描述符复制到用户的发送缓冲区
...
void UG_USB_NoData_Setup0(void)
{
  ...
  if(USB_Type_Recipient == (USB_STANDARD_REQUEST | USB_DEVICE_RECIPIENT))
  {
    ...
    {Result = UG_Standard_SetConfiguration();}
    ...
    else if (DUSB.Device_Info.USBbRequest == USB_SET_FEATURE)//SET FEATURE for Device
    {
      ...
      {Result = UG_Standard_SetDeviceFeature();}
      else
    ...
    else if (DUSB.Device_Info.USBbRequest == USB_CLEAR_FEATURE)//Clear FEATURE for Device
    {
      ...
      {Result = UG_Standard_ClearFeature();}
      else
      {Result = USB_UNSUPPORT;}
    }
  }
  else if (USB_Type_Recipient == (USB_STANDARD_REQUEST | USB_INTERFACE_RECIPIENT))//Interface Request
  {if (DUSB.Device_Info.USBbRequest == USB_SET_INTERFACE) Result = UG_Standard_SetInterface();}//SET INTERFACE
  else if (USB_Type_Recipient == (USB_STANDARD_REQUEST | USB_ENDPOINT_RECIPIENT))//EndPoint Request
  {
    ...
    {Result = UG_Standard_SetEndPointFeature();}
  }
  else
  {Result = USB_UNSUPPORT;}
  ...
  {
    Result =UG_CustomHID_NoData_Setup(DUSB.Device_Info.USBbRequest);
  ...
  UG_USB_SendZeroLength();  //这句很关键，实际在准备好发送0字节的状态数据包。然后等待in令牌包
  return;
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_USB_Data_Setup0(void)//获取描述符的函数：设备、配置、字符串描述符,放在 *CopyRoutine
{
  UINT8 *(*CopyRoutine)(UINT16);
  USB_RESULT Result;
  CopyRoutine = NULL;
  ...
  else if ((DUSB.Device_Info.USBbRequest == USB_GET_STATUS) && (DUSB.Device_Info.USBwValues.VAL == 0x0000)  && (DUSB.Device_Info.USBwLengths.VAL == 0x0002) && (DUSB.Device_Info.USBwIndexs.LH.H == 0x00))
  {
    ...
    {CopyRoutine = UG_Standard_GetStatus;}
    else if (USB_Type_Recipient == (USB_STANDARD_REQUEST | USB_INTERFACE_RECIPIENT))
    {if ((UG_CustomHID_Get_Interface_Setting(DUSB.Device_Info.USBwIndexs.LH.L, 0) == USB_SUCCESS)  && DUSB.Device_Info.Current_Configuration)CopyRoutine = UG_Standard_GetStatus;}
    else if (USB_Type_Recipient == (USB_STANDARD_REQUEST | USB_ENDPOINT_RECIPIENT))
    {
  ...
  {if (USB_Type_Recipient == (USB_STANDARD_REQUEST | USB_DEVICE_RECIPIENT))CopyRoutine = UG_Standard_GetConfiguration; }
  else if (DUSB.Device_Info.USBbRequest == USB_GET_INTERFACE)
  {
    ...
    && (DUSB.Device_Info.USBwIndexs.LH.H == 0) && (DUSB.Device_Info.USBwLengths.VAL == 0x0001) && (UG_CustomHID_Get_Interface_Setting(DUSB.Device_Info.USBwIndexs.LH.L, 0) == USB_SUCCESS))
    {CopyRoutine = UG_Standard_GetInterface;}
  }
  ...
  {Result =UG_CustomHID_Data_Setup(DUSB.Device_Info.USBbRequest);  if(Result == USB_NOT_READY){DUSB.Device_Info.ControlState = PAUSE; return;}}
  ...
  if (DUSB.Device_Info.USBbmRequestType&0x80)	//此时应为0x80 ，bmRequnest的第四位分别是选择设备、接口、端点的
  {
    ...
    UG_USB_DataStageIn();		  //最主要的就是调用这个函数完成描述符的输出准备
  }
  else
...
UINT8 UG_USB_Setup0_Process(void)
{
  if (DUSB.Device_Info.ControlState != PAUSE)
  {
		...
		DUSB.Device_Info.USBwIndexs.VAL=USB.RXBUF0[2];  //wIndex
  ...
  if (DUSB.Device_Info.USBwLengths.VAL)  UG_USB_Data_Setup0(); else  UG_USB_NoData_Setup0();//Setup with data stage,获取描述符的函数：设备、配置、字符串描述符
  
  return UG_USB_Post0_Process();
}
//------------------------------------------------------------------------------------------------------------------------------
UINT8 UG_USB_In0_Process(void)
{
  ...
  else if (DUSB.Device_Info.ControlState == WAIT_STATUS_IN)
  {
    ...
    {UG_USB_SetAddress(DUSB.Device_Info.USBwValues.LH.L);	DUSB.DEVState=USB_STATE_ADDRESSED;}
    UG_CustomHID_Status_In();
...
UINT8 UG_USB_Out0_Process(void)
{
  ...
  {UG_CustomHID_Status_Out(); DUSB.Device_Info.ControlState = STALLED;}
...
{
  UG_USB_SetRXCount(0, DUSB.EPSIZE[0]);//将端点0的缓冲区大小设置
...
{
  USB.CH[0].TXCOUNT=0x00; DUSB.TXSTAT=USB_EPR_STAT_VALID;
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_USB_CTR_LP(void)//端点低优先级中断处理
{
	while(USB.ISTR_BIT.CTR)	
  {
    ...
    if (DUSB.EPIndex == 0)
    {
			DUSB.RXSTAT=USB.EPR_BIT[0].RXSTAT; DUSB.TXSTAT=USB.EPR_BIT[0].TXSTAT;UG_USB_SetRXTXSTAT(0,USB_EPR_STAT_NAK,USB_EPR_STAT_NAK);			
			if(!USB.ISTR_BIT.DIR)//如果DIR＝0，相应端点的CTR_TX位被置位，标志一个IN分组(数据从USB模块传输到PC主机)
		  {USB.EPR[0]&=0x8F0F;UG_USB_In0_Process();}
      else//如果DIR＝1，相应端点的CTR_RX位被置位，标志一个OUT分组(数据从PC主机传输到USB模块)的传输完成。
      {USB.EPR[0]&=0x0F8F;if(USB.EPR_BIT[0].SETUP){UG_USB_Setup0_Process();} else {if (USB.EPR_BIT[0].RXCTR)UG_USB_Out0_Process(); else return;}}
		  UG_USB_SetRXTXSTAT(0,DUSB.RXSTAT,DUSB.TXSTAT);
    }
    else//EPindex ！= 0
    {
			if(USB.EPR_BIT[DUSB.EPIndex].RXCTR)
      {USB.EPR[DUSB.EPIndex]&=0x0F8F; if(DUSB.EPIndex==USB_RXEPIndex)UG_USB_ReadBUF(DUSB.EPIndex);UG_USB_SetRXSTAT(DUSB.EPIndex,USB_EPR_STAT_VALID);}
...
{
  if(USB.ISTR_BIT.CTR && USB.CNTR_BIT.CTRM){USB.ISTR_BIT.CTR=0;UG_USB_CTR_LP();}
  if(USB.ISTR_BIT.RST && USB.CNTR_BIT.RSTM){USB.ISTR_BIT.RST=0;UG_CustomHID_Reset();}
  if(USB.ISTR_BIT.DOVR && USB.CNTR_BIT.DOVRM){USB.ISTR_BIT.DOVR=0;UG_USB_OVR_CallBack();}
  if(USB.ISTR_BIT.ERR && USB.CNTR_BIT.ERRM){USB.ISTR_BIT.ERR=0;UG_USB_ERR_CallBack();}
  if(USB.ISTR_BIT.WKUP && USB.CNTR_BIT.WKUPM){USB.ISTR_BIT.WKUP=0;UG_USB_Resume(USB_RESUME_EXTERNAL);}
  if(USB.ISTR_BIT.SUSP && USB.CNTR_BIT.SUSPM){USB.ISTR_BIT.SUSP=0;if(USB_Suspend_EN)UG_USB_Suspend();else  {USB.CNTR_BIT.SUSPM=0;UG_USB_Resume(USB_RESUME_LATER);}}
  if(USB.ISTR_BIT.SOF && USB.CNTR_BIT.SOFM){USB.ISTR_BIT.SOF=0;USB.CNTR_BIT.SOFM=0;UG_USB_Resume(USB_RESUME_SOF);}
  if(USB.ISTR_BIT.ESOF && USB.CNTR_BIT.ESOFM){USB.ISTR_BIT.ESOF=0;USB.CNTR_BIT.ESOFM=0;UG_USB_Resume(USB_RESUME_ESOF);} // request without change of the machine state 
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_USB_PowerOn(void)
{
  USB_SetPullUp(1);USB.CNTR_BIT.FRES =1;USB_CNTR=0x0000;USB_ISTR=0x0000;USB_CNTR=0x1C00;//Force Reset;clear interrup;clear pending;RSTM=SUSPM=WKUPM=1
}
//------------------------------------------------------------------------------------------------------------------------------
void UG_USB_PowerOff(void)
{
	USB_CNTR=0x0001;USB_ISTR=0x0000;USB_CNTR=0x0003;USB_SetPullUp(0);
}
//------------------------------------------------------------------------------------------------------------------------------
...
void UG_USB_Resume(USB_RESUME_STATE eResumeSetVal)
{
  ...
  switch (DUSB.ResumeS.eState)//
  {
    case USB_RESUME_EXTERNAL:
      UG_USB_Resume_Init(); DUSB.ResumeS.eState = USB_RESUME_OFF; break;
    case USB_RESUME_INTERNAL:
      UG_USB_Resume_Init(); DUSB.ResumeS.eState = USB_RESUME_START; break;
...
void UG_USB_ReadBUF(UINT8 EPIndex)
{
  ...
  DAPP.INTF.COM_ReadParameter(SDK_USB_Index,DCOM.Item[SDK_USB_Index].RF.Item,DCOM.Item[SDK_USB_Index].RF.ParaCount);
}	
//------------------------------------------------------------------------------------------------------------------------------
void UG_USB_SendBUF(UINT8 EPIndex)//发送BUF（中断内外调用）
{
  ...
  {for(i=0;i<DCOM.Item[SDK_USB_Index].TF.MMT.Count-2;i++)DCOM.Item[SDK_USB_Index].TF.MMT.Item[i]=DCOM.Item[SDK_USB_Index].TF.MMT.Item[i+1];DCOM.Item[SDK_USB_Index].TF.MMT.Count--;}
	...
	USB.CH[EPIndex].TXCOUNT=(USB_DMLenth_EN ? Len:DUSB.EPSIZE[USB_TXEPIndex]);	UG_USB_SetTXSTAT(EPIndex, USB_EPR_STAT_VALID);
}
//------------------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------------------
void UG_USB_HW_SendString(UINT8 *s,UINT16 Count)//发送到硬件（中断内外调用）
{
	UINT16 i;UINT32 T;
  ...
  T=SysTimeCount;while(USB.EPR_BIT[USB_TXEPIndex].TXSTAT!=USB_EPR_STAT_NAK && SysTimeCount-T<2); 
	...
	USB.CH[USB_TXEPIndex].TXCOUNT=(USB_DMLenth_EN ? Count:DUSB.EPSIZE[USB_TXEPIndex]);UG_USB_SetTXSTAT(USB_TXEPIndex, USB_EPR_STAT_VALID);
...
//------------------------------------------------------------------------------------------------------------------------------
void UG_USB_TXTimeOut_Timer(void)//every 100us,send timeout control
{
...
void UG_USB_Send_Timer(void)//发送定时事件,every 1ms(中断内外调用）
{
  ...
  if(DCOM.Item[SDK_USB_Index].TF.Count==0x00||DCOM.Item[SDK_USB_Index].TF.MMT.Item[0].Time)UG_COM_EndSend(SDK_USB_Index); else UG_USB_SendBUF(USB_TXEPIndex);
}
//------------------------------------------------------------------------------------------------------------------------------
#endif
//==============================================================================================================================
